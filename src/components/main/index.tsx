...
export default function Index({ cat, setPagin }: { cat: CategoryType, setPagin: React.Dispatch<SetStateAction<{ isNext: boolean, isPrev: boolean }>>, }) {
    ...
    if (data && data.filter(el => el.value?.length).length && data.filter(el => el.value?.length).length == activedCatIndex) {
        setPagin((prev) => ({ ...prev, isNext: true }))
        setActivedCatIndex(0)
    } else if (data && activedCatIndex < 0) {
        setPagin((prev) => ({ ...prev, isPrev: true }))
        setActivedCatIndex(0)
    }
    return (
        <div className='flex w-full flex-col h-[92vh] main-container '>
            {
            ...
            }
            {
                ...
                >
                    ...
                    <SwiperSlide className='w-full h-full'>
                        <TextSpace classn={'relative pt-14'} line={true} text={locale == 'ru' ? 'Переход на предыдущую категорию' : 'Мурунку категорияга өтүү'} />
                    </SwiperSlide>
                    ...
                    <SwiperSlide className='w-full h-full'>
                        <TextSpace line={true} text={locale == 'ru' ? 'Переход на следующую категорию' : 'Кийинки категорияга өтүү'} />
                    </SwiperSlide>
                </Swiper> : null
            }
            {
