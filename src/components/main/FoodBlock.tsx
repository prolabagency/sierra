...
export default function ({ category, setChangeItem }: { category: SubcatType, setChangeItem: React.Dispatch<SetStateAction<FoodsType | null>> }) {
    ...
    return (
        <div className={`w-full mb-10`}>
            <TextSpace classn='text-wrapped' text={locale == 'ru' ? category.title || category.title_ky : category.title_ky || category.title} />
            {
                activedItem ?
            ...
            <Swiper
                className='m-0 w-full h-full flex justify-center items-center'
                slidesPerView={1}
                initialSlide={0}
                centeredSlides
